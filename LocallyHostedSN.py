...
class Network:
    ...
    def create_user(self, username, password):
        if username not in self.users:
            ...
            self.users[username] = {'password': hashed_password, 'user': User(username)}
    ...
    def add_friendship(self, username, friend):
        if username in self.users and friend in self.users:
            user = self.users[username]['user']
            friend_user = self.users[friend]['user']
        ...
        try:
            with open("user_data.json", "r") as file:
                ...
                for username, data in user_data.items():
                    ...
                    self.users[username] = {'password': data['password'], 'user': new_user}
...
class LoginWindow:
    def __init__(self, master, network):
        ...
        self.label_password = tk.Label(master, text="Enter Password:", font=("Arial", 16), bg="black", fg="white")
        self.entry_password = tk.Entry(master, font=("Arial", 16), show="*")
        ...
        self.button_login.grid(row=2, column=0, columnspan=2, padx=10, pady=10)

    def login(self):
        username = self.entry_username.get()
        password = self.entry_password.get()
        if self.network.login_user(username, password):
            self.master.withdraw()
            account_window = AccountWindow(tk.Toplevel(self.master), self.network, self.master)
            account_window.set_current_user(username)

class AccountWindow:
    def __init__(self, master, network, login_window):
        self.master = master
        ...
        self.button_logout = tk.Button(master, text="Logout", command=self.logout, bg="red", fg="black", font=("Arial", 16))
        ...
        self.button_logout.grid(row=5, column=0, columnspan=2, padx=10, pady=10)
    ...
    def display_friends_info(self):
        if self.current_user:
            user = self.network.users[self.current_user]['user']
...
if __name__ == "__main__":
    ...
    network = Network()
    login_window = LoginWindow(root, network)
    root.mainloop()
